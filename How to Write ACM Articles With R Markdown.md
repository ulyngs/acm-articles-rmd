...
If we add `keep_tex: true` in the YAML header, indented under `pdf_document:`, then we can knit again and inspect the intermediary .tex file that the PDF is generated from.
...
Turns out this is easy. Go to the YAML header and under `pdf_document:` add (with indentation) `citation_package: natbib`. This will make use of the LaTeX natbib package to insert the citations into the .tex, instead of inserting them as plain text. Let's knit again to see if it worked:
